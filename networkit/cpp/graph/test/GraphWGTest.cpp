...
namespace NetworKit {
class GraphWGTest : public testing::TestWithParam<std::tuple<bool, bool>> {
public:
    virtual void SetUp();
...
INSTANTIATE_TEST_SUITE_P(InstantiationName, GraphWGTest,
                         testing::Values(std::make_tuple(false, false),
                                         std::make_tuple(true, false), std::make_tuple(false, true),
                                         std::make_tuple(true, true)));

bool GraphWGTest::isWeighted() const {
    return std::get<0>(GetParam());
}
bool GraphWGTest::isDirected() const {
    return std::get<1>(GetParam());
}

GraphW GraphWGTest::createGraphW(count n) const {
...
GraphW GraphWGTest::createGraphW(count n, count m) const {
...
count GraphWGTest::countSelfLoopsManually(const GraphW &G) {
    count c = 0;
...
TEST(GraphWGTest, testDefConstructorWithUndirIndex) {
...
TEST(GraphWGTest, testDefConstructorWithDirIndex) {
...
TEST_P(GraphWGTest, testCopyConstructorWithIndexedEdgeIds) {
...
TEST_P(GraphWGTest, DISABLED_testCopyConstructor) {
...
TEST_P(GraphWGTest, testAddNode) {
...
TEST_P(GraphWGTest, testAddNodes) {
...
TEST_P(GraphWGTest, testRemoveNode) {
...
TEST_P(GraphWGTest, testRemovingANodeLeavesAHoleInTheIdSpace) {
    GraphW G = ErdosRenyiGenerator(20, 0.3, false).generate();
    ASSERT_TRUE(G.hasContiguousNodeIds());

    G.removeNode(5);
    EXPECT_FALSE(G.hasContiguousNodeIds());

    G.restoreNode(5);
    EXPECT_TRUE(G.hasContiguousNodeIds());
}

TEST_P(GraphWGTest, testHasNode) {
    GraphW G = createGraphW(5);
...
TEST_P(GraphWGTest, testRestoreNode) {
...
TEST_P(GraphWGTest, testIsIsolated) {
...
TEST_P(GraphWGTest, testWeightedDegree) {
    // add self-loop
...
TEST_P(GraphWGTest, testWeightedDegree2) {
    // add self-loop
    this->Ghouse.addEdge(2, 2, 0.75);
...
TEST_P(GraphWGTest, testWeightedDegree3) {
...
TEST_P(GraphWGTest, testAddEdge) {
...
TEST_P(GraphWGTest, testRemoveEdge) {
    double epsilon = 1e-6;
    GraphW G = createGraphW(3);
...
TEST_P(GraphWGTest, testRemoveAllEdges) {
    constexpr count n = 100;
...
TEST_P(GraphWGTest, testRemoveSelfLoops) {
    constexpr count n = 100;
...
TEST_P(GraphWGTest, testRemoveMultiEdges) {
...
TEST_P(GraphWGTest, testSelfLoopCountSimple) {
    GraphW G(Ghouse);
    G.addEdge(0, 0);
    EXPECT_EQ(1, G.numberOfSelfLoops());
}

TEST_P(GraphWGTest, testIsEmpty) {
...
TEST_P(GraphWGTest, testNumberOfNodes) {
    GraphW G1 = createGraphW(0);
...
TEST_P(GraphWGTest, testNumberOfEdges) {
...
TEST_P(GraphWGTest, testNumberOfSelfLoops) {
    GraphW G = createGraphW(3);
    G.addEdge(0, 1);
...
TEST_P(GraphWGTest, testSelfLoopConversion) {
...
TEST_P(GraphWGTest, testUpperNodeIdBound) {
    GraphW G1 = createGraphW(0);
...
TEST_P(GraphWGTest, testCheckConsistency_MultiEdgeDetection) {
    GraphW G = createGraphW(3);
...
TEST_P(GraphWGTest, testSetWeight) {
...
TEST_P(GraphWGTest, increaseWeight) {
    GraphW G = createGraphW(5);
    G.addEdge(0, 1);
    G.addEdge(1, 2);
    G.addEdge(3, 4, 3.14);
...
TEST_P(GraphWGTest, testTotalEdgeWeight) {
    GraphW G1 = createGraphW(5);
...
TEST_P(GraphWGTest, testNodeIterator) {
    ...
    auto testForward = [](const GraphW &G) {
        ...
        for (const auto u : GraphW::NodeRange(G)) {
    ...
    auto testBackward = [](const GraphW &G) {
        const std::vector<node> nodes(GraphW::NodeRange(G).begin(), GraphW::NodeRange(G).end());
        std::vector<node> v;
...
TEST_P(GraphWGTest, testEdgeIterator) {
    GraphW G(this->Ghouse);
    ...
    auto testForward = [&](const GraphW &G) {
        ...
        G1 = G;
        for (const auto edge : GraphW::EdgeRange(G)) {
    ...
    auto testForwardWeighted = [&](const GraphW &G) {
        ...
        G1 = G;
        for (const auto edge : GraphW::EdgeWeightRange(G)) {
            ASSERT_TRUE(G1.hasEdge(edge.u, edge.v));
...
TEST_P(GraphWGTest, testForNodes) {
    GraphW G = createGraphW(3);
...
TEST_P(GraphWGTest, forNodesWhile) {
    count n = 100;
    GraphW G = createGraphW(n);
    count stopAfter = 10;
    count nodesSeen = 0;
...
TEST_P(GraphWGTest, testForNodesInRandomOrder) {
    count n = 1000;
    count samples = 100;
...
TEST_P(GraphWGTest, testForNodePairs) {
    count n = 10;
    count m = n * (n - 1) / 2;
    GraphW G = createGraphW(n);
...
TEST_P(GraphWGTest, testForWeightedInEdgesOf) {
    // add self-loop
...
TEST_P(GraphWGTest, testEdgeIndexGenerationDirected) {
...
TEST_P(GraphWGTest, testEdgeIndexGenerationUndirected) {
...
TEST_P(GraphWGTest, testEdgeIndexResolver) {
    GraphW G = createGraphW(10);
    G.indexEdges();
...
TEST_P(GraphWGTest, testForEdgesWithIds) {
...
TEST_P(GraphWGTest, testForWeightedEdgesWithIds) {
    std::vector<GraphW> graphs;
    graphs.emplace_back(10, false, false);
    graphs.emplace_back(10, false, true);
    graphs.emplace_back(10, true, false);
...
TEST_P(GraphWGTest, testParallelForEdgesWithIds) {
    std::vector<GraphW> graphs;
    graphs.emplace_back(10, false, false);
    graphs.emplace_back(10, false, true);
    graphs.emplace_back(10, true, false);
...
TEST_P(GraphWGTest, testParallelForWeightedEdgesWithIds) {
    std::vector<GraphW> graphs;
    graphs.emplace_back(10, false, false);
    graphs.emplace_back(10, false, true);
    graphs.emplace_back(10, true, false);
...
/*TEST_P(GraphWGTest, testInForEdgesUndirected) {
...
TEST_P(GraphWGTest, testSortEdges) {
    GraphW G = this->Ghouse;
...
TEST_P(GraphWGTest, testEdgeIdsSortingAfterRemove) {
...
TEST_P(GraphWGTest, testEdgeIdsConsistencyAfterRemove) {
    constexpr node n = 100;
...
TEST_P(GraphWGTest, testEdgeIdsAfterRemoveWithoutSortingOrIDs) {
    constexpr node n = 100;
...
TEST(GraphWGTest, testSortNeighborsUndirectedGraph) {
    GraphW G(6);
    G.addEdge(0, 3);
    G.addEdge(0, 5);
    G.addEdge(0, 4);
...
TEST(GraphWGTest, testSortNeighborsUndirectedIndexedGraph) {
    GraphW G(6);
    G.addEdge(0, 3);
    G.addEdge(0, 5);
    G.addEdge(0, 4);
...
TEST(GraphWGTest, testSortNeighborsWeightedUndirectedGraphByWeights) {
    GraphW G(6, true);
    G.addEdge(0, 3, 9.0);
    G.addEdge(0, 5, 7.0);
    G.addEdge(0, 4, 8.0);
...
TEST(GraphWGTest, testSortNeighborsDirectedGraph) {
...
TEST(GraphWGTest, testSortNeighborsWeightedDirectedGraphByWeights) {
...
TEST(GraphWGTest, testSortNeighborsWeightedDirectedIndexedGraph) {
