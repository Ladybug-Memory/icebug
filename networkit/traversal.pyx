...
cdef class Traversal:
	...
	def BFSfrom(Graph graph, start, object callback):
		...
		try:
			...
			try:
				...
				sources = [<node?>start]
			BFSfrom[vector[node].iterator, TraversalNodeDistCallbackWrapper](dereference(graph._view()), sources.begin(),sources.end(), dereference(wrapper))
		finally:
			del wrapper
	...
	def BFSEdgesFrom(Graph graph, node start, object callback):
		...
		try:
			...
			BFSEdgesFrom[TraversalEdgeCallBackWrapper](dereference(graph._view()), start, dereference(wrapper))
		finally:
			del wrapper
	...
	def DFSfrom(Graph graph, node start, object callback):
		...
		try:
			...
			DFSfrom[TraversalNodeCallbackWrapper](dereference(graph._view()), start, dereference(wrapper))
		finally:
			del wrapper
	...
	def DFSEdgesFrom(Graph graph, node start, object callback):
		...
		try:
			wrapper = new TraversalEdgeCallBackWrapper(callback)
			DFSEdgesFrom[TraversalEdgeCallBackWrapper](dereference(graph._view()), start, dereference(wrapper))
		finally:
			del wrapper
	...
	def DijkstraFrom(Graph graph, start, object callback):
		...
		try:
			...
			try:
				...
				sources = [<node?>start]
			DijkstraFrom[vector[node].iterator, TraversalNodeDistCallbackWrapper](dereference(graph._view()), sources.begin(),sources.end(), dereference(wrapper))
		finally:
			del wrapper
